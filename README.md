...
3. 将得到的授权码代入到以下url地址中，此处用postman进行操作：[http://client:secret@localhost:8085/oauth/token?grant_type=authorization_code&code=prgYij&redirect_uri=http://www.baidu.com](http://client:secret@localhost:8085/oauth/token?grant_type=authorization_code&code=prgYij&redirect_uri=http://www.baidu.com) 携带授权码，得到`access_token`，该`access_token`在前面拼接上`Bearer `即为jwt令牌。
